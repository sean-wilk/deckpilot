...
import { validateCard } from '@/lib/ai/card-validation'
...
export async function POST(request: Request) {
  try {
    ...
    const { commanderId, description, targetBracket, budgetLimitCents, spiciness, generationMode = 'fast' } = body
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          async function processEvents(parsed: { type: string; data: unknown }[]) {
            for (const evt of parsed) {
              if (evt.type === 'card' && generationMode === 'fast') {
                // Inline validation during fast mode streaming
                const result = await validateCard(
                  (evt.data as { name: string }).name,
                  colorIdentity,
                  seenCardIds
                )
                if (result.valid && result.cardId) {
                  seenCardIds.add(result.cardId)
                  validCount++
                } else {
                  invalidCount++
                }
          ...
          // Emit validation summary after streaming completes (fast mode only)
          if (generationMode === 'fast') {
            controller.enqueue(encoder.encode(formatSSE('validation_summary', {
              valid: validCount,
